...
"""
    ...
    def emitAttributes(self, entity):
        ...
        for attr in others:
            self.file.write("   .. attribute:: {2}.{0}\n\n      type: :class:`{1}`\n\n".format(attr.get_name(), attr.get_type().__str__().replace("::", "."),entity.get_name()))
    ...
    def emitModule(self, module):
        ...
        with open(self.dir+"/"+module.get_full_name().replace("::", "_") + ".rst", 'w') as file:
            self.file = file;
    ...
    def emitComment(self, context, comment):
        ...
        if(re.search(r":param (\w+) (\w*)([^:]*)(?:$|\n)", comment)):
            warn("DEPRECATION! :param in {0} should be of the form ':param name type: message'. The second ':' is missing".format(context))
...
def export_doc(exporter, types):
    dir = Config.get("doc","outdir","doc")
    if not os.path.exists(dir):
        os.mkdir(dir)
    
    with open(dir+'/index.rst', 'w') as file:
        em = Emitter(dir,file)
